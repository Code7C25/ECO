...
function PanelPsicologoScreen({ route, navigation }) {
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity 
        style={styles.button} 
        onPress={() => navigation.navigate("FormAnonimos")}
      >
        <Text style={styles.buttonText}>Formularios Anónimos</Text>
      </TouchableOpacity>

      <TouchableOpacity 
        style={styles.button} 
        onPress={() => navigation.navigate("FormPublicos")}
      >
        <Text style={styles.buttonText}>Formularios Públicos</Text>
      </TouchableOpacity>
    </View>
  );
}
...
function FormAnonimosScreen({ navigation }) {
  const [formularios, setFormularios] = useState([]);

  useEffect(() => {
    const cargarFormularios = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "formulariosAnonimos"));
        const lista = [];
        querySnapshot.forEach((docu) => {
          lista.push({ id: docu.id, ...docu.data() });
        });
  ...
  return (
    <View style={styles.container}>
      <BackButton navigation={navigation} />
      <Text style={styles.title}>Formularios Anónimos</Text>
      {formularios.length === 0 ? (
        <Text style={styles.subtitle}>No hay formularios aún</Text>
      ) : (
        formularios.map((f) => (
          <Text 
            key={f.id} 
            style={styles.link}
            onPress={() => Linking.openURL(f.link)}
          >
            {f.link}
          </Text>
        ))
...
function FormPublicosScreen({ navigation }) {
  const [formularios, setFormularios] = useState([]);

  useEffect(() => {
    const cargarFormularios = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "formulariosPublicos"));
        const lista = [];
        querySnapshot.forEach((docu) => {
          lista.push({ id: docu.id, ...docu.data() });
        });
  ...
  return (
    <View style={styles.container}>
      <BackButton navigation={navigation} />
      <Text style={styles.title}>Formularios Públicos</Text>
      {formularios.length === 0 ? (
        <Text style={styles.subtitle}>No hay formularios aún</Text>
      ) : (
        formularios.map((f) => (
  ...
  return (
    <NavigationContainer>
      <Stack.Navigator screenOptions={{ headerShown: false }}>
        ...
        <Stack.Screen name="FormAnonimos" component={FormAnonimosScreen} />
        <Stack.Screen name="FormPublicos" component={FormPublicosScreen} />
      </Stack.Navigator>
    </NavigationContainer>
  );
}
